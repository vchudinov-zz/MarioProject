...
/**
 * Created by IntelliJ IDEA. User: Nikolay Sohryakov,
 * nikolay.sohyrakov@gmail.com Date: Nov 28, 2010 Time: 12:21:34 AM Package:
...
public class WaveGoomba extends Enemy {
	...
	public WaveGoomba(LevelScene world, int x, int y, int dir, int mapX,
			int mapY) {
			...
			if (deadTime == 0) {
				...
				for (int i = 0; i < 8; i++) {
					world.addSprite(new Sparkle(
							...
							(int) (y - Math.random() * 8) + 4, (float) (Math
									.random() * 2 - 1), (float) Math.random()
									* -1, 0, 1, 5));
				}
