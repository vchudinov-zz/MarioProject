...
/**
 * The method that you need to invoke is Writer. It takes as an argument the
 * address to the genome file
 * 
 * @author Viktor
 * 
 */
public class GraphVizWriter {
	...
	public void writer(String genome, String targetfile, String graphName)
			throws IOException {
	 ...
	 * 
	 * @param file
	 *            - the name of the file. It will be automatically put in the
	 *            relevant folder
	 * @param ranksep
	 *            - how wide the layers will appear from each other in inches.
	 *            3-4 is good for now
	 * @throws IOException
	 */
	private void writeGene(String file, int ranksep, String graphName)
			throws IOException {
	 ...
	 * 
	 * @param pw
	 *            - PrinterWriter deals with writing the information to file
	 * @param label
	 *            - the label of the layer see the enum in this class.
	 * @param nodes
	 *            - the set of nodes that go in the layer
	 */
	public void layerWriter(PrintWriter pw, layer label, HashSet<String> nodes) {
		...
		for (String s : nodes) {
			if (!s.startsWith(bias))
				...
				pw.println(s
						+ " [style=filled,color= gold, shape=circle,height = 2, width = 2, fontsize = 64]");
		}
