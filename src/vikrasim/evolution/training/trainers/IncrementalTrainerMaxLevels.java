...
public class IncrementalTrainerMaxLevels extends AverageTrainer {
	...
	public IncrementalTrainerMaxLevels(String parameterFileName, String debugParameterFileName, String genomeFileName, 
	...
	public boolean trainNetwork(String[] trainingSets, double winnerPercentageThreshold, MasterAgent agent, int maxDifficulty) throws IOException{
		boolean status;
		...
		experimentSession(agent, numberOfGenerations, trainingSets, winnerPercentageThreshold, maxDifficulty);
	...
	private void experimentSession (MasterAgent agent, int generations, String[] trainingSets, double winnerPercentageThreshold, int maxDifficulty) throws IOException{
		...
		for (int expCount = 0; expCount < Neat.p_num_runs; expCount++){
			runExperiment(starterGenome, generations, trainingSets, winnerPercentageThreshold, maxDifficulty);
		}
		
	}
	private void runExperiment(Genome starterGenome, int maxGenerations, String[] trainingSets, double winnerPercentageThreshold, int maxDifficulty) throws IOException{
		String mask6 = "000000";
		...
		do {
			...
			winnerFolder = rootWinnerFolder + delimiter + "Dif " + difficultyLevel + " levels " + levelsTrainingOn;
			testAndCreate(winnerFolder);
			
			generationInfoFolder = rootGenerationInfoFolder + delimiter + "Dif " + difficultyLevel + " levels " + levelsTrainingOn;
			testAndCreate(generationInfoFolder);
			
			System.out.print("\n---------------- E P O C H  < " + gen +" >--------------");
			String filenameEpochInfo = "g_" + fmt6.format(gen);
			String[] levelsToTrain = buildTrainingSet(trainingSets, difficultyLevel, levelsTrainingOn);
	...
	private String[] buildTrainingSet(String[] allSets, int difficultyLevel, int numberOfLevels){
