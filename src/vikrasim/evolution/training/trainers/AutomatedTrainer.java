...
public class AutomatedTrainer {
	...
	public boolean trainNetwork(String[] scenarios, int maxNumberOfNoImprovement) throws IOException{
		...
		for (int i = 0; i < scenarios.length; i++){
			...
			//Find starter genome
			if (i == 0){
				genomeFileName = this.starterGenomeFileName;
			}else {
				genomeFileName = winnerFolder +  delimiter + nameOfExperiment + " gen " + lastGeneration + " best";
			}			
			...
			lastGeneration = experimentSession(genomeFileName, numberOfGenerations, stopOnFirstGoodOrganism, maxNumberOfNoImprovement);
		}
