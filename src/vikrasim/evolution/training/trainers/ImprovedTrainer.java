...
public class ImprovedTrainer {
	...
	public ImprovedTrainer(String parameterFileName, String debugParameterFileName, String genomeFileName, 
			...
			AboveAverageEvaluator evaluator, String delimiter){
	...
	public ImprovedTrainer(){
		
	}
	
	public boolean trainNetwork(String[] trainingSet, double winnerPercentageThreshold,int runsPerLevel, int maxDifficulty) throws IOException{
		boolean status;
		...
		experimentSession(starterGenomeFileName, numberOfGenerations, trainingSet, winnerPercentageThreshold,runsPerLevel,maxDifficulty);
	 ...
	 * @throws IOException 
	 */
	private void experimentSession (String starterGenomeFileName, int generations, String[] trainingSets, double winnerPercentageThreshold, int runsPerLevel, int maxDifficulty) throws IOException{
		...
		if (ret){
			...
			for (int expCount = 0; expCount < Neat.p_num_runs; expCount++){
				runExperiment(starterGenome, generations, trainingSets, winnerPercentageThreshold, runsPerLevel, maxDifficulty);
			}
	 ...
	 * @throws IOException
	 */
	private void runExperiment(Genome starterGenome, int maxGenerations, String[] trainingSets, double winnerPercentageThreshold, int runsPerLevel, int maxDifficulty) throws IOException{
		String mask6 = "000000";
		...
		int levelIndex = 0;
		int difficulty = 1;
		int maxRuns = runsPerLevel;
		int gen = 1;
		
		do {
			//Create folder to save winners and generation info
			winnerFolder = rootWinnerFolder + delimiter + " Level " + levelIndex;
			testAndCreate(winnerFolder);
			
			generationInfoFolder = rootGenerationInfoFolder + delimiter + "Level " + levelIndex;
			testAndCreate(generationInfoFolder);
			
			System.out.print("\n---------------- E P O C H < " + gen +" > --------------");
			String filenameEpochInfo = "g_" + fmt6.format(gen);
			String level = trainingSets[levelIndex] + " -ld " + difficulty;
			int numberOfWinners = goThroughEpoch(pop, gen, filenameEpochInfo, level, 5);
			if (numberOfWinners > 0) {
				if (enoughWinnersInPopulation(pop, winnerPercentageThreshold, numberOfWinners)){
					difficulty ++;
					System.out.println();
					System.out.println("Changing difficulty to " + difficulty);
					System.out.println();
				}
			}
			if (difficulty > maxDifficulty && levelIndex < trainingSets.length)
			{	System.out.println("Moving to next level");
				difficulty = 1;
				levelIndex++;
			}
			gen++;
		} while (gen <= maxGenerations && levelIndex < trainingSets.length);
	...
	private int goThroughEpoch(Population pop, int generation, String filenameEpochInfo, String level, int runs) throws IOException{
		...
		while (itr_organism.hasNext()){
			...
			//evaluate 
			status = evaluator.evaluate(curOrganism, level, runs);
