...
public class RewardNoJumpEvaluator extends MasterEvaluator {

	public RewardNoJumpEvaluator(String levelParameters, MasterAgent agent) {
	...
	private boolean runSimulation(Organism organism, String[] trainingSet){
	    ...
	    for (int i = 0; i < trainingSet.length; i++){
		    ...
		    if (status== Mario.STATUS_WIN){
		    	time = ev[11]; //Time left
			    mode = ev[7]+1;
		    	organism.setWinner(true);
		    	localFitness += time + mode * 100;
		    } 
		    else{
