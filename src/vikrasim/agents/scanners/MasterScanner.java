...
public abstract class MasterScanner {
	...
	public MasterScanner(int length, int height, Dir scanDirection,
			ScannerType type) {
		if (!checkSize(length))
			return;
		if (!checkSize(height))
	...
	protected abstract void populatelLine(int[][] cellsToScan, int marioX,
			int marioY);

	/**
	 * Finds all the cells in the lenght x height area with mario in the corner
	 * |M| | | | | | | Example with height = 3 and length = 3, scanning SE | | |
	 * |
	 * 
	 * @param cellsToScan
	 * @param marioX
	...
	public double scan(byte[][] enemies, byte[][] levelScene) {
		...
		switch (type) {
		case ENEMY:
			result = scanNumberOfObjects(enemies);
			break;
		case ENVIRONMENT:
			result = scanDistanceToObject(levelScene);
			break;
		}
